...
class AStar(object):
    ...
    def get_neighbors(self, x):
        del1 = [-self.resolution,0, self.resolution]
        deltas = list(it.product(del1, del1))
        free=[]
        for delx, dely in deltas:
            if (delx, dely)==(0,0):
    ...
    def solve(self):
        while len(self.open_set)>0:
            ...
            #process: look at neighbors
            for xn in self.get_neighbors(xc):
                if xn in self.closed_set: #skip if processed 
                    continue
                g_temp=self.g_score[xc]+ self.distance(xc, xn) #stage cost and cost2go
                if xn not in self.open_set: #add to open if haven't seen 
                    self.open_set.append(xn)
                elif g_temp > self.g_score[xn]: #path is worse skip
                    continue
                self.came_from[xn]= xc #build backtrace 
                #update scores 
